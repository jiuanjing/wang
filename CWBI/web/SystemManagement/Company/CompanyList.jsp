<%
    /*********************************************************************************************************************
     *程序名称：CompanyList.jsp
     *功能描述：以列表的形式显示当前所有的单位/公司
     *编写人：万怀
     *编写时间：2014-09-12
     **********************************************************************************************************************/
%>
...
<%
    ...
    String positionStr = "单位信息管理";//当前位置
    String programName = "单位";//当前功能名称
    //打开数据库链接
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr = "select company_id,company_code \"单位编号\",company_name \"单位名称\",brief_name \"简称\",order_no \"排序号\",company_level \"组织级别\" from odccbim.company";

            //取检索条件
            String where = "";
            if (request.getParameter("CompanyCode") != null && request.getParameter("CompanyCode").trim().length() > 0) {
                where = where + " where company_code like '%" + ChStr.chStr(request.getParameter("CompanyCode").trim()) + "%'";
                if (request.getParameter("CompanyName") != null && request.getParameter("CompanyName").trim().length() > 0) {
                    where = where + " and company_name like '%" + ChStr.chStr(request.getParameter("CompanyName").trim()) + "%'";
                }
            } else {
                if (request.getParameter("CompanyName") != null && request.getParameter("CompanyName").trim().length() > 0) {
                    where = where + " where company_name like '%" + ChStr.chStr(request.getParameter("CompanyName").trim()) + "%'";
                }
            }
...
<script>
    function goPage(p) {
        ...
        tab_list.action = "CompanyList.jsp";
        tab_list.submit();
    }
    ...
    {
        tab_list.CompanyCode.value = "";
        tab_list.CompanyName.value = "";
    }
    ...
    function deleteChoosedItems() {
        ...
        document.tab_list.action = "CompanyCtl.jsp";
                ...
                <tr>
                    <td width="94%" align="left" valign="top">
                        <table id="table1" border="0" cellspacing="0" align="center" width="95%">
                            ...
                            <tr>
                                <td>
                                    <form name="tab_list" method="POST" action="CompanyList.jsp">
                                        ...
                                        <table id="Table2" class="tab1">
                                                ...
                                                <td colspan="8">
                                                    <div id="frmTitle" style="display:none">
                                                        <table id="t1" class="tab1" width="100%">
                                                            ...
                                                            <tr>
                                                                <td>单位编号： <input name="CompanyCode" type="text" size=12
                                                                                 value=""/>
                                                                    单位名称：<input name="CompanyName" type="text" size=12
                                                                                value=""/>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td height="21" align="right">
                                                                ...
                                                                <input name="addNew" type="button" class="button"
                                                                       value="新建"
                                                                       onclick="javascript:self.location='CompanyItf.jsp'">
                                            ...
                                            <tr  <%=bgcolor%> onclick='setRowIndex(this)'>
                                                ...
                                                <td align="center" nowrap><a
                                                        href='CompanyItf.jsp?CompanyID=<%=rs.getString(1)%>'><%=rs.getString(col)%>
                                                </a></td>
                                                <%
                                                } else {%>
