...
<%!
    ...
    public boolean service(String Type, DBOperation db) {
        ...
        if (Type.equals("insert")) {
            Vector<Vector<String>> v = db.executeQueryVt("select nvl(max(module_id),0)+1 from odccbim.module");
            if (v == null || v.size() == 0) {
    ...
    if (db.dbOpen()) {
        try {
            if (type.equals("deleteChoosed")) {
                int re = 0;
                String sqlstr = "";
                sqlstr = "delete from odccbim.module where module_id in(" + selectedIds + ")";
