...
<%!
    ...
    public boolean service(String Type, DBOperation db) {
        ...
        if (Type.equals("insert")) {
            Vector<Vector<String>> v = db.executeQueryVt("select nvl(max(user_id),0)+1 from odccbim.user_info");
            if (v == null || v.size() == 0) {
    ...
    if (db.dbOpen()) {
        try {
            if (type.equals("deleteChoosed")) {
                int re = 0;
                String sqlstr = "";
                sqlstr = "delete from odccbim.user_info where user_id in(" + selectedIds + ")";
