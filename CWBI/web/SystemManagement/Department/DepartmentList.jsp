<%
    /*********************************************************************************************************************
     *程序名称：DepartmentList.jsp
     *功能描述：以列表的形式显示当前所有的部门
     *编写人：万怀
     *编写时间：2014-09-12
     **********************************************************************************************************************/
%>
...
<%
    ...
    String positionStr = "部门信息管理";//当前位置
    String programName = "部门";//当前功能名称
    //打开数据库链接
    if (db.dbOpen()) {
        try {
            ...
            //取检索条件
            String where = "";
            if (request.getParameter("DeptCode") != null && request.getParameter("DeptCode").trim().length() > 0) {
                where = where + " where dept_code like '%" + ChStr.chStr(request.getParameter("DeptCode").trim()) + "%'";
                if (request.getParameter("DeptName") != null && request.getParameter("DeptName").trim().length() > 0) {
                    where = where + " and dept_name like '%" + ChStr.chStr(request.getParameter("DeptName").trim()) + "%'";
                }
            } else {
                if (request.getParameter("DeptName") != null && request.getParameter("DeptName").trim().length() > 0) {
                    where = where + " where dept_name like '%" + ChStr.chStr(request.getParameter("DeptName").trim()) + "%'";
                }
            }
...
<script>
    function goPage(p) {
        ...
        tab_list.action = "DepartmentList.jsp";
        tab_list.submit();
    }
    ...
    {
        tab_list.DeptCode.value = "";
        tab_list.DeptName.value = "";
    }
    ...
    function deleteChoosedItems() {
        ...
        document.tab_list.action = "DepartmentCtl.jsp";
                ...
                <tr>
                    <td width="94%" align="left" valign="top">
                        <table id="table1" border="0" cellspacing="0" align="center" width="95%">
                            ...
                            <tr>
                                <td>
                                    <form name="tab_list" method="POST" action="DepartmentList.jsp">
                                        ...
                                        <table id="Table2" class="tab1">
                                                ...
                                                <td colspan="8">
                                                    <div id="frmTitle" style="display:none">
                                                        <table id="t1" class="tab1" width="100%">
                                                            ...
                                                            <tr>
                                                                <td>部门编号： <input name="DeptCode" type="text" size=12
                                                                                 value=""/>
                                                                    部门名称：<input name="DeptName" type="text" size=12
                                                                                value=""/>
                                            ...
                                            <tr  <%=bgcolor%> onclick='setRowIndex(this)'>
                                                ...
                                                <td align="center" nowrap><a
                                                        href='DepartmentItf.jsp?DeptID=<%=rs.getString(1)%>'><%=rs.getString(col)%>
                                                </a></td>
                                                <%
                                                } else {%>
