    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr = "select "
                    ...
                    + "from ( "
                    + "select a.sum_id," + column + " all_cy from echarts.dm_op_mr_kpi_actual_sum a where a.date_id = " + date2 + " and a.sum_id = " + DeptID + " and a.kpi_code_num = " + kpiCodeNum
                    + ") v101 "
                    + "left outer join "
                    + "( "
                    + "select a.sum_id," + column + " all_ly from echarts.dm_op_mr_kpi_actual_sum a where a.date_id = " + date1 + " and a.sum_id = " + DeptID + " and a.kpi_code_num = " + kpiCodeNum
                    + ") v102 "
                    + "on (v101.sum_id = v102.sum_id) "
                    + "left outer join "
                    + "( "
                    + "select a.sum_id," + column + " all_budget from echarts.dm_op_mr_kpi_budget_sum a where a.date_id = " + date2 + " and a.sum_id = " + DeptID + " and a.kpi_code_num = " + kpiCodeNum
                    + ") v103 "
