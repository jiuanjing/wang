...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr1 = "select nvl(round(sum" + month + ",2),0) from echarts.dm_op_mr_kpi_actual t where t.company_id =" + CompanyID + " and t.kpi_code_num=1034 and t.date_id = " + year;
            String sqlstr2 = "select nvl(round(sum" + month + ",2),0) from echarts.dm_op_mr_kpi_budget t where t.company_id = " + CompanyID + " and t.kpi_code_num=1034 and t.date_id = " + year;
            ...
            map1.put("value", d1);
            map1.put("name", "产销差率");//实际值
            list.add(map1);
            Map<String, Object> map2 = new HashMap<String, Object>();
            map2.put("value", d2);
            map2.put("name", "产销差率");//预算值
            a_list.add(map2);
