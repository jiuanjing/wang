...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr1 = "select " + column + "," + sumcolumn + " from echarts.dm_op_mr_kpi_actual t where t.company_id = " + CompanyID + " and t.kpi_code_num=2050 and t.date_id = " + (year - 1);//同期平均水价
            String sqlstr2 = "select " + column + "," + sumcolumn + " from echarts.dm_op_mr_kpi_actual t where t.company_id = " + CompanyID + " and t.kpi_code_num=2050 and t.date_id = " + year;//平均水价实际值
            String sqlstr3 = "select " + column + "," + sumcolumn + " from echarts.dm_op_mr_kpi_budget t where t.company_id = " + CompanyID + " and t.kpi_code_num=2050 and t.date_id = " + year;//平均水价计划
            ...
            map1.put("kpi", "平均水价量同期值(元/吨)");
            map2.put("kpi", "平均水价量实际值(元/吨)");
            map3.put("kpi", "平均水价量预算值(元/吨)");
