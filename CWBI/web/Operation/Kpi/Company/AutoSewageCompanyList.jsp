<%
     ...
     *功能描述：从数据库中动态取得数据，并返回  用于模糊查询污水公司名称
     *编写人：cjl
     **********************************************************************************************************************/
%>
...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            if (deptId > 0) {
                sqlstr = "select a.company_id , a.company_name from echarts." + tableName + " a where a.status = 1 and a.flag_sewage = 1 and a.dept_id = " + deptId + " order by a.order_no";
            } else {
                sqlstr = "select a.company_id , a.company_name from echarts." + tableName + " a where a.status = 1 and a.flag_sewage = 1 order by a.order_no";
            }
