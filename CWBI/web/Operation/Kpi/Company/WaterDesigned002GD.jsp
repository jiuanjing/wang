...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            String colume = "";
            for (int i = 0; i < 12; i++) {
                colume += "nvl(round(m" + (i + 1) + ",2),0),";
            }
            colume = colume.substring(0, colume.length() - 1).trim();
            //累计值
            String sumColume = "nvl(round(m13,2),0)";

            String sqlstr1 = "select " + colume + "," + sumColume + " from echarts.dm_op_mr_kpi_actual t where t.company_id = " + CompanyID + " and t.kpi_code_num=1059 and t.date_id = " + (year - 1);//同期设计水量
            String sqlstr2 = "select " + colume + "," + sumColume + " from echarts.dm_op_mr_kpi_actual t where t.company_id = " + CompanyID + " and t.kpi_code_num=1059 and t.date_id = " + year;//设计水量实际值
