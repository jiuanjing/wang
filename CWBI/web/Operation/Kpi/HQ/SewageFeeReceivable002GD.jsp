...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            String column = "";
            String sqlstr1 = "select nvl(round(sum(sum" + month + "),2),0) from echarts.dm_op_mr_kpi_actual_sum t where t.kpi_code_num=2001 and  t.sum_id = 0 and t.date_id = " + year;//实际应收水费
            String sqlstr2 = "select nvl(round(sum(sum" + month + "),2),0) from echarts.dm_op_mr_kpi_budget_sum t where t.kpi_code_num=2001 and  t.sum_id = 0 and t.date_id = " + year;//计划应收水费
