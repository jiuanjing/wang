...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select c.bu_name,round(sum(a.operating_revenue_ty)/10000,2) from echarts.dm_operating_growth a ,echarts.dim_company_fn b,echarts.dim_bu c "
            ...
            sqlstr = sqlstr + where + " and b.status=1 and b.company_level=2 and c.status=1 and c.analysis_flag=1 group by c.bu_name order by sum(a.operating_revenue_ty) desc";
            ...
            map1.put("value", 18233.23);
            map1.put("name", "供水板块");
            Map<String, Object> map2 = new HashMap<String, Object>();
            map2.put("value", 16126.77);
            map2.put("name", "污水板块");
            list.add(map1);
            list.add(map2);
