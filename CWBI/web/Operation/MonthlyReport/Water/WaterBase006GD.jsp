    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select t.year_id,"
                    ...
                    + "t.estimate_roi,"
                    + "t.roi"
                    + " from echarts.dm_op_mr_basic_fin_w t where 1=1";
            String where = "";
            ...
            if (intRowCount > 0) {
                ...
                while (!rs.isAfterLast()) {
                    ...
                    map.put("register_capital", rs.getString(2));
                    map.put("estimate_roi", rs.getString(3));
                    map.put("roi", rs.getString(4));
                    list_a.add(map);
                    rs.next();
                }
            } else {
                for (int i = 0; i < 3; i++) {
                    Map<String, Object> map = new HashMap<String, Object>();
                    map.put("year_id", "");
                    map.put("register_capital", "");
                    map.put("estimate_roi", "");
                    map.put("roi", "");
                    list_a.add(map);
                }
            }
