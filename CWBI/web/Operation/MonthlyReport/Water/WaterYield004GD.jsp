    ...
    if (db.dbOpen()) {
        try {
            ...
            if (!("".equals(date)) && date != null) {
				...
				/* 预算值 */
            String sqlstr1 = "select a.kpi_id,a.kpi_name \"计划售水量\",a.kpi_unit,"
                    ...
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1027";
				/* 本期实际值 */
            String sqlstr2 = "select a.kpi_id,a.kpi_name \"实际售水量\",a.kpi_unit,"
                    + " nvl(b.m1,0),nvl(b.m2,0),nvl(b.m3,0),nvl(b.m4,0),nvl(b.m5,0),"
                    + " nvl(b.m6,0),nvl(b.m7,0),nvl(b.m8,0),nvl(b.m9,0),nvl(b.m10,0),"
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1028";
				/* 去年同期值 */
            String sqlstr3 = "select a.kpi_id,a.kpi_name \"去年同期售水量\",a.kpi_unit,"
                    + " nvl(b.m1,0),nvl(b.m2,0),nvl(b.m3,0),nvl(b.m4,0),nvl(b.m5,0),"
                    + " nvl(b.m6,0),nvl(b.m7,0),nvl(b.m8,0),nvl(b.m9,0),nvl(b.m10,0),"
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + _date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1029";
            ...
            //查询原因
            String sqlstr = "";
            sqlstr = "select REASON_WATER_SELL from echarts.dm_op_mr_reason_w t where 1=1";
