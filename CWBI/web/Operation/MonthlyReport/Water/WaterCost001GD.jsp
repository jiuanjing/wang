    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr1 = "select a.kpi_id,a.kpi_name \"平均电价\",a.kpi_unit,"
                    ...
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num =1036";
				/* 本期实际值 */
            String sqlstr2 = "select a.kpi_id,a.kpi_name \"电量\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num =1037";
				/* 本期实际值 */
            String sqlstr3 = "select a.kpi_id,a.kpi_name \"单位制水电耗\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num =1038";
				/* 预算值 */
            String sqlstr4 = "select a.kpi_id,a.kpi_name \"预算单位制水电耗\",a.kpi_unit,"
                    ...
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1039";
				/* 去年同期值 */
            String sqlstr5 = "select a.kpi_id,a.kpi_name \"去年同期单位制水电耗\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + _date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1040";
            ...
            //查询原因
            String sqlstr = "";
            sqlstr = "select REASON_COST_POWER from echarts.dm_op_mr_reason_w t where 1=1";
