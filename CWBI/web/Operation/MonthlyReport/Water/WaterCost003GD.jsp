    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr1 = "select a.kpi_id,a.kpi_name \"污泥处置成本\",a.kpi_unit,"
                    ...
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1045";
				/* 预算值 */
            String sqlstr2 = "select a.kpi_id,a.kpi_name \"预算污泥处置成本\",a.kpi_unit,"
                    ...
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1046";
				/* 去年同期值 */
            String sqlstr3 = "select a.kpi_id,a.kpi_name \"去年同期污泥处置成本\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + _date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num = 1047";
				/* 本期实际值 */
            String sqlstr4 = "select a.kpi_id,a.kpi_name \"污泥产量\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num =1048";
				/* 本期实际值 */
            String sqlstr5 = "select a.kpi_id,a.kpi_name \"单位污泥处置成本\",a.kpi_unit,"
                    ...
                    + " nvl(b.m11,0),nvl(b.m12,0),nvl(b.m13,0),nvl(b.m14,0)"
                    + " from echarts.dim_op_kpi a left join echarts.dm_op_mr_kpi_actual b on a.kpi_code_num = b.kpi_code_num"
                    + " where b.date_id =" + date + " and b.company_id =" + CompanyID + " and a.status = 1 and a.kpi_code_num =1049";
            ...
            //查询原因
            String sqlstr = "";
            sqlstr = "select REASON_COST_SLUDGE from echarts.dm_op_mr_reason_w t where 1=1";
