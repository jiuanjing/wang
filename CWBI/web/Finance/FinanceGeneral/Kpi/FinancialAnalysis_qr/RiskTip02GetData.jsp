...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select t.company_id,t.brief_name,t.COST_PROFIT_RATIO from (select a.COMPANY_ID,b.brief_name,a.COST_PROFIT_RATIO from echarts.dm_qr_profitability a,echarts.dim_company_fn b where a.company_id = b.company_id";

            String where = "";
            where += " and a.ROE <> 0 and a.company_id > 2";
            ...
            sqlstr = sqlstr + where + " and b.status=1 order by a.COST_PROFIT_RATIO desc) t where rownum<=10";
            ...
            if (intRowCount > 0) {
                ...
                while (!rs.isAfterLast()) {
                    ...
                    map.put("cost_profit_ratio", rs.getString(3));
                    list.add(map);
                    rs.next();
                }
            }
