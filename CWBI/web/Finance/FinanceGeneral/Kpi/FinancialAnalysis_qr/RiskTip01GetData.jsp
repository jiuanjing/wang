...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select t.company_id,t.brief_name,t.ROE,t.PROFIT_CW from (select a.COMPANY_ID,b.brief_name,a.ROE,a.PROFIT_CW from echarts.dm_qr_profitability a,echarts.dim_company_fn b where a.company_id = b.company_id";
            ...
            sqlstr = sqlstr + where + " and b.status=1 order by a.ROE asc) t where rownum<=10";
            ...
            if (intRowCount > 0) {
                ...
                while (!rs.isAfterLast()) {
                    ...
                    map.put("profit_cw", rs.getString(4));
                    list.add(map);
                    rs.next();
                }
            }
