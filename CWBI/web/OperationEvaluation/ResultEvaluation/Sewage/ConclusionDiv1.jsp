...
<% 
	...
	if(db.dbOpen()){
		...
		String sql = "select t1.brief_name," +
		                "       sum(score) as score," +
		                "       sum(decode(t.kpi_id, 2100, t.actual_value, null)) as 盈利能力," +
		                "       sum(decode(t.kpi_id, 2200, t.actual_value, null)) as 债务风险," +
		                "       sum(decode(t.kpi_id, 2300, t.actual_value, null)) as 现金获取," +
		                "       sum(decode(t.kpi_id, 2400, t.actual_value, null)) as 成本控制," +
		                "       sum(decode(t.kpi_id, 2500, t.actual_value, null)) as 业务增长," +
		                "       sum(decode(t.kpi_id, 2600, t.actual_value, null)) as 经验效率," +
		                "       sum(decode(t.kpi_id, 2700, t.actual_value, null)) as 人员素质" +
