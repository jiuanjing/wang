...
<%
    ...
    if (db.dbOpen()) {
        try {
            String sqlstr = "";
            String bu_name = request.getParameter("bu_name").trim();
            ...
            sqlstr = "select t.bu_id,t.bu_name,t.bu_code,t.bu_level, u.bu_name,t.status,t.order_no from echarts.dim_bu t left join echarts.dim_bu u on u.bu_id = t.uplevel_id";

            String where = "";
            if (bu_name != null && !(bu_name.equals(""))) {
                String name = new String(bu_name.getBytes("ISO8859-1"), "UTF-8");
                where += " where t.bu_name like '%" + name + "%'";
            }
            ...
            if (intRowCount > 0) {
                ...
                for (int i = 0; i < rows; i++) {
                    ...
                    map.put("bu_id", rs.getString(1));
                    map.put("bu_name", rs.getString(2));
                    map.put("bu_code", rs.getString(3));
                    map.put("bu_level", rs.getString(4));
                    map.put("uplevel_name", rs.getString(5));
                    map.put("status", rs.getString(6));
                    map.put("order_no", rs.getString(7));
                    list.add(map);
                    if (rs.isLast()) {
                        break;
                    } else {
